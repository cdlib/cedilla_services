...
require('./services/lc_authority_service.rb')

class OclcLcAuthority < Sinatra::Application
  
  default = "The Oclc LC Authority File service is expecting an HTTP POST with a JSON message similar to the examples found on: " +
  ...
  # -------------------------------------------------------------------------
  post '/lc_authority' do
    headers['Content-Type'] = 'text/json'
    payload = ""
    service = LcAuthorityService.new
    ...
    begin  
      ...
      puts data
      puts citation
      puts citation.authors.size
      
      begin
        if citation.authors.first.nil?
